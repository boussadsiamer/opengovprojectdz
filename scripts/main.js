...
function setupEventListeners() {
    ...
    // Dark mode toggle (smooth + persistent)
    ...
    if (darkModeToggle) {
        darkModeToggle.addEventListener('click', () => {
            const current = (localStorage.getItem('theme') === 'dark' || document.documentElement.getAttribute('data-theme') === 'dark') ? 'dark' : 'light';
            const next = current === 'dark' ? 'light' : 'dark';
            applyTheme(next);
        });

        // Initialize theme on load: prefer saved, then user preference, then light
